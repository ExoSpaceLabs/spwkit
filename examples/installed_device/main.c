...
static int wait_ready(spw_port_t* port, spw_ready_events_t interest) {
    spw_ready_events_t ready = SPW_READY_NONE;
    return spw_port_wait(port, interest, EXAMPLE_TIMEOUT_US, &ready) == SPW_OK &&
           (ready & interest) != 0u;
}
...
static int receive_bytes(spw_port_t* port,
    ...
    return wait_ready(port, SPW_READY_RX_PACKET) &&
           spw_port_receive(port, &packet, EXAMPLE_TIMEOUT_US) == SPW_OK &&
...
int main(int argc, char** argv) {
    ...
    printf("installed C device peer %lu completed\n", port_id);
    result = 0;

...
#define SPW_PROFILE_RX_COMPARISON_MAX_SAMPLES 4096u
#define SPW_PROFILE_RX_COMPARISON_MAX_PAYLOAD 4096u
#define SPW_PROFILE_RX_COMPARISON_WORKSPACE_SIZE 4096u
...
typedef union comparison_workspace {
    ...
    uint8_t bytes[SPW_PROFILE_RX_COMPARISON_WORKSPACE_SIZE];
...
static uint64_t g_native_samples[SPW_PROFILE_RX_COMPARISON_MAX_SAMPLES];
static uint64_t g_spwkit_samples[SPW_PROFILE_RX_COMPARISON_MAX_SAMPLES];
static uint8_t g_source[SPW_PROFILE_RX_COMPARISON_MAX_PAYLOAD];
static uint8_t g_native_buffer[SPW_PROFILE_RX_COMPARISON_MAX_PAYLOAD];
static uint8_t g_spwkit_buffer[SPW_PROFILE_RX_COMPARISON_MAX_PAYLOAD];
...
static spw_result_t comparison_get_capabilities(
    ...
    out_capabilities->max_packet_size = SPW_PROFILE_RX_COMPARISON_MAX_PAYLOAD;
...
static spw_result_t comparison_send(void* raw,
...
static spw_result_t comparison_provider_receive(comparison_driver_t* driver,
                                                spw_packet_t* packet) {
    ...
    /* Shared provider/native receive boundary for both measured paths. */
    SPW_PROFILE_RX_PROVIDER_BOUNDARY();

    if (driver->payload_length != 0u) {
        memcpy(packet->data, g_source, driver->payload_length);
...
static spw_result_t comparison_driver_receive(void* raw,
                                              spw_packet_t* packet,
                                              spw_timeout_us_t timeout_us) {
    (void)timeout_us;
    return comparison_provider_receive((comparison_driver_t*)raw, packet);
}

static spw_result_t comparison_native_receive(comparison_driver_t* driver,
                                              spw_packet_t* packet,
                                              spw_timeout_us_t timeout_us) {
    spw_result_t result;
    (void)timeout_us;
    result = comparison_provider_receive(driver, packet);
...
static const spw_driver_ops_t COMPARISON_DRIVER_OPS = {
    ...
    .send = comparison_send,
    .receive = comparison_driver_receive,
};
...
static int run_native_sample(comparison_driver_t* driver,
                             ...
                             uint64_t* out_delta) {
    packet->length = 0u;
    if (comparison_native_receive(driver, packet, SPW_TIMEOUT_IMMEDIATE) != SPW_OK) {
        return 0;
    }
...
static void print_result(size_t payload_size,
    ...
    const long long p95_delta = (long long)spwkit_statistics->p95 - (long long)native_statistics->p95;
    const long long p99_delta = (long long)spwkit_statistics->p99 - (long long)native_statistics->p99;
    ...
    printf(",\"boundary\":\"provider-native-boundary-to-api-return\"");
    printf(",\"provider_fixture\":\"shared-reference-provider-receive\"");
    ...
    if (native_statistics->median != 0.0) {
        printf(",\"median_percent\":%.3f", 100.0 * median_delta / native_statistics->median);
    } else {
        printf(",\"median_percent\":null");
    }
    if (native_statistics->mean != 0.0) {
        printf(",\"mean_percent\":%.3f", 100.0 * mean_delta / native_statistics->mean);
    } else {
...
static void print_usage(const char* program) {
    fprintf(stderr, "Usage: %s [--warmup N] [--iterations N] [--payload N]\n", program);
}
...
int main(int argc, char** argv) {
    ...
    spw_packet_t native_packet;
    spw_packet_t spwkit_packet;
    ...
    for (i = 1u; i < (size_t)argc; ++i) {
        if (strcmp(argv[i], "--warmup") == 0 && i + 1u < (size_t)argc) {
            ...
            if (!parse_size(argv[++i], 1u, SPW_PROFILE_RX_COMPARISON_MAX_SAMPLES, &iterations)) {
                print_usage(argv[0]);
                return 2;
            }
        } else if (strcmp(argv[i], "--payload") == 0 && i + 1u < (size_t)argc) {
            if (!parse_size(argv[++i], 0u, SPW_PROFILE_RX_COMPARISON_MAX_PAYLOAD, &payload_size)) {
                print_usage(argv[0]);
                return 2;
            }
        } else {
    ...
    driver.state = SPW_LINK_READY;
    driver.payload_length = payload_size;
    for (i = 0u; i < payload_size; ++i) {
        g_source[i] = (uint8_t)((i * 17u + 3u) & 0xffu);
    }

    memset(&driver_config, 0, sizeof(driver_config));
    driver_config.struct_size = sizeof(driver_config);
    driver_config.version = SPW_DRIVER_CONFIG_VERSION;
    driver_config.ops = &COMPARISON_DRIVER_OPS;
    driver_config.driver_context = &driver;
    ...
    if (spw_port_workspace_requirements(&config, &requirements) != SPW_OK ||
        ...
        requirements.alignment > _Alignof(comparison_workspace_t)) {
        fprintf(stderr, "failed to resolve comparison workspace\n");
        return 1;
    }
    if (spw_port_open_in_place(&config, g_workspace.bytes, sizeof(g_workspace.bytes), &port) != SPW_OK ||
        spw_port_start(port) != SPW_OK) {
        fprintf(stderr, "failed to open/start comparison port\n");
        return 1;
    }

    memset(&native_packet, 0, sizeof(native_packet));
    native_packet.data = g_native_buffer;
    native_packet.capacity = sizeof(g_native_buffer);
    memset(&spwkit_packet, 0, sizeof(spwkit_packet));
    spwkit_packet.data = g_spwkit_buffer;
    spwkit_packet.capacity = sizeof(g_spwkit_buffer);

    spw_profile_prepare();
    for (i = 0u; i < warmup_iterations; ++i) {
        uint64_t ignored;
        if (!run_native_sample(&driver, &native_packet, &ignored) ||
            !run_spwkit_sample(port, &spwkit_packet, &ignored)) {
            fprintf(stderr, "warmup receive failed\n");
            return 1;
        }
    }
    ...
    for (i = 0u; i < iterations; ++i) {
        int ok;
        if ((i & 1u) == 0u) {
            ok = run_native_sample(&driver, &native_packet, &g_native_samples[i]) &&
                 run_spwkit_sample(port, &spwkit_packet, &g_spwkit_samples[i]);
        } else {
            ok = run_spwkit_sample(port, &spwkit_packet, &g_spwkit_samples[i]) &&
                 run_native_sample(&driver, &native_packet, &g_native_samples[i]);
        }
        if (!ok) {
            fprintf(stderr, "measured receive failed at iteration %zu\n", i);
            return 1;
        }
    }

    if (payload_size != 0u &&
        (memcmp(g_native_buffer, g_source, payload_size) != 0 ||
         memcmp(g_spwkit_buffer, g_source, payload_size) != 0)) {
        fprintf(stderr, "received payload mismatch\n");
        return 1;
    }
    ...
    print_result(payload_size, warmup_iterations, iterations,
                 &native_statistics, &spwkit_statistics);

    if (spw_port_stop(port) != SPW_OK || spw_port_close(port) != SPW_OK) {
        fprintf(stderr, "failed to stop/close comparison port\n");
        return 1;
    }
    return 0;
}

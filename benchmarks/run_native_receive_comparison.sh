...
output=""
calibration_output=""
build_dir="$ROOT_DIR/build/profile-native-rx-comparison"
settle_seconds="1"

usage() {
  cat <<'EOF'
Usage: benchmarks/run_native_receive_comparison.sh [options]

Runs one clean Release build of the direct/provider versus SpWKit copied DRIVER
RX comparison. Both paths execute in the same process and share the exact
provider receive function.
...
printf '[native-rx-comparison] removing build tree: %s\n' "$build_dir" >&2
rm -rf -- "$build_dir"
...
cmake --build "$build_dir" \
  --target spwkit_profile_native_receive_comparison spwkit_profile_counter_floor \
  --parallel 2

binary="$build_dir/spwkit_profile_native_receive_comparison"
...
if (( settle_seconds > 0 )); then
  printf '[native-rx-comparison] settling for %ss before timing\n' "$settle_seconds" >&2
  sleep "$settle_seconds"
fi

calibration_line="$($floor_binary --warmup "$warmup" --iterations "$iterations")"
printf '[native-rx-comparison] diagnostic counter floor: %s\n' "$calibration_line" >&2

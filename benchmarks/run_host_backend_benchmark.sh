...
output=""
calibration_output=""
build_dir="$ROOT_DIR/build/profile-host-backend"
settle_seconds="1"

usage() {
  cat <<'EOF'
Usage: benchmarks/run_host_backend_benchmark.sh --backend NAME --direction tx|rx [options]

Runs one clean Release benchmark build for an in-memory hosted backend. The
measured public API direction has all internal paired probes compiled to no-ops;
the opposite probe domain remains selected only to expose the common
architectural counter implementation.
...
Options:
  ...
  --output PATH         write backend JSON Lines to PATH as well as stdout
...
case "$backend" in
  loopback) build_simulator=OFF ;;
  simulator) build_simulator=ON ;;
  *) echo "backend must be loopback or simulator" >&2; exit 2 ;;
esac
...
printf '[host-backend] %s/%s: removing build tree: %s\n' "$backend" "$direction" "$build_dir" >&2
rm -rf -- "$build_dir"
...
cmake -S "$ROOT_DIR/benchmarks" -B "$build_dir" \
  ...
  -DSPWKIT_BENCHMARK_BUILD_SIMULATOR="$build_simulator"
cmake --build "$build_dir" \
  --target spwkit_profile_host_backend spwkit_profile_counter_floor \
  --parallel 2

binary="$build_dir/spwkit_profile_host_backend"
...
if (( settle_seconds > 0 )); then
  printf '[host-backend] %s/%s: settling for %ss before timing\n' "$backend" "$direction" "$settle_seconds" >&2
  sleep "$settle_seconds"
fi

calibration_line="$($floor_binary --warmup "$warmup" --iterations "$iterations")"
printf '[host-backend] %s/%s diagnostic counter floor: %s\n' "$backend" "$direction" "$calibration_line" >&2
...
for payload in "${payload_array[@]}"; do
  line="$($binary --backend "$backend" --direction "$direction" \
    --warmup "$warmup" --iterations "$iterations" --payload "$payload")"
  printf '%s\n' "$line"
  if [[ -n "$output" ]]; then
    printf '%s\n' "$line" >> "$output"
  fi

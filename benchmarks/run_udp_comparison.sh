...
output=""
calibration_output=""
build_dir="$ROOT_DIR/build/profile-udp-comparison"
settle_seconds="1"

usage() {
  cat <<'EOF'
Usage: benchmarks/run_udp_comparison.sh --direction tx|rx [options]

Runs one clean Release comparison build for VSPW-TP/UDP against a direct
same-process loopback UDP socket path. Queue draining and cooperative ACK
service happen outside the measured interval.
...
printf '[udp-comparison] %s: removing build tree: %s\n' "$direction" "$build_dir" >&2
rm -rf -- "$build_dir"
...
cmake --build "$build_dir" \
  --target spwkit_profile_udp_comparison spwkit_profile_counter_floor \
  --parallel 2

binary="$build_dir/spwkit_profile_udp_comparison"
floor_binary="$build_dir/spwkit_profile_counter_floor"
for executable in "$binary" "$floor_binary"; do
  if [[ ! -x "$executable" ]]; then
    echo "UDP comparison executable was not produced: $executable" >&2
    exit 1
  fi
done

if (( settle_seconds > 0 )); then
  printf '[udp-comparison] %s: settling for %ss before timing\n' "$direction" "$settle_seconds" >&2
  sleep "$settle_seconds"
fi

calibration_line="$($floor_binary --warmup "$warmup" --iterations "$iterations")"
printf '[udp-comparison] %s diagnostic counter floor: %s\n' "$direction" "$calibration_line" >&2
...
for payload in "${payload_array[@]}"; do
  line="$($binary --direction "$direction" --warmup "$warmup" \
    --iterations "$iterations" --payload "$payload")"
  printf '%s\n' "$line"
  if [[ -n "$output" ]]; then
    printf '%s\n' "$line" >> "$output"
  fi

...
output=""
calibration_output=""
build_dir="$ROOT_DIR/build/profile-zero-copy-comparison"
settle_seconds="1"

usage() {
  cat <<'EOF'
Usage: benchmarks/run_zero_copy_comparison.sh [options]

Runs one clean Release build of the deterministic DRIVER copied-versus-zero-copy
TX fixture. Both paths use the same provider-owned DMA-like storage and stop at
the same provider/native submission boundary. No physical transfer is included.
...
printf '[zero-copy-comparison] removing build tree: %s\n' "$build_dir" >&2
rm -rf -- "$build_dir"

# Select an RX probe pair that the TX fixture never emits. Profiling remains
# enabled solely to expose the architectural counter helpers used by the raw
# comparison fixture; no selected paired probe contributes to these samples.
cmake -S "$ROOT_DIR/benchmarks" -B "$build_dir" \
  -DCMAKE_BUILD_TYPE=Release \
  -DSPWKIT_BENCHMARK_PROFILE_START=SPW_PROFILE_ID_RX_PROVIDER_BOUNDARY \
  -DSPWKIT_BENCHMARK_PROFILE_END=SPW_PROFILE_ID_RX_API_RETURN \
  -DSPWKIT_BENCHMARK_COUNTER_HZ="$counter_hz"
cmake --build "$build_dir" \
  --target spwkit_profile_zero_copy_comparison spwkit_profile_counter_floor \
  --parallel 2

binary="$build_dir/spwkit_profile_zero_copy_comparison"
floor_binary="$build_dir/spwkit_profile_counter_floor"
for executable in "$binary" "$floor_binary"; do
  if [[ ! -x "$executable" ]]; then
    echo "zero-copy comparison executable was not produced: $executable" >&2
    exit 1
  fi
done

if (( settle_seconds > 0 )); then
  printf '[zero-copy-comparison] settling for %ss before timing\n' "$settle_seconds" >&2
  sleep "$settle_seconds"
fi

calibration_line="$($floor_binary --warmup "$warmup" --iterations "$iterations")"
printf '[zero-copy-comparison] diagnostic counter floor: %s\n' "$calibration_line" >&2

...
BUILD_ROOT="${SPWKIT_STM32_NATIVE_BUILD_ROOT:-$ROOT_DIR/build/stm32-native-comparison}"
...
PROFILE_CASES=(copied_tx zc_tx_submit copied_rx zc_rx_acquire)

usage() {
  cat <<'USAGE'
Usage:
  scripts/stm32h755_native_compare_campaign.sh --stm32h7-root /path/to/STM32CubeH7 [options]

Runs paired direct-provider versus SpWKit public-API comparisons on a physical
NUCLEO-H755ZI-Q. Native and SpWKit order alternates within each measured row.

Options:
  --stm32h7-root DIR     pinned STM32CubeH7 checkout root
  --cases "LIST"        all or space/comma-separated comparison cases
  --warmup N            paired warmup iterations per payload (default: 16)
  --iterations N        paired measured iterations, 1..256 (default: 64)
  ...
  --list-cases           print supported comparison cases
...
for selected in "${selected_cases[@]}"; do
  ...
  (( found == 1 )) || { echo "unknown native comparison case: $selected" >&2; exit 2; }
done
...
case_config() {
  case "$1" in
    ...
    zc_tx_submit)
      CASE_ID=10
      START=SPW_PROFILE_ID_TX_ZC_SUBMIT_API_ENTRY
      END=SPW_PROFILE_ID_TX_ZC_SUBMIT_PROVIDER_BOUNDARY
      ;;
    copied_rx)
      CASE_ID=11
    ...
    zc_rx_acquire)
      CASE_ID=12
      START=SPW_PROFILE_ID_RX_ZC_ACQUIRE_PROVIDER_BOUNDARY
      END=SPW_PROFILE_ID_RX_ZC_ACQUIRE_API_RETURN
      ;;
    *) return 1 ;;
  esac
}
...
result_name="stm32h755-native-${timestamp}-${git_short}"
...
archive_results() {
  ...
  if [[ ! -s "$archive" ]]; then
    printf 'ERROR: archive is missing/empty: %s\n' "$archive" >&2
    rm -f -- "$archive"
    return 1
  fi
  printf 'Archive       : %s\n' "$archive" >&2
...
fail_campaign() {
  ...
  if [[ -n "${raw_build:-}" && -s "${raw_build:-}" && -n "${raw_out:-}" ]]; then
    cp "$raw_build" "$raw_out" || true
  fi
  if [[ -d "$output_dir" ]]; then
    local failure_archive="${output_dir}.failed.tar"
    printf 'Partial results : %s\n' "$output_dir" >&2
    archive_results "$failure_archive" || true
  fi
  exit "$status"
}

printf 'SpWKit STM32H755 direct/native differential campaign\n' >&2
printf '  commit: %s\n' "$git_sha" >&2
...
printf '  order: alternating native-first / SpWKit-first\n' >&2
printf '  cases: %s\n' "${selected_cases[*]}" >&2
printf '  results: %s\n' "$output_dir" >&2
...
for case_name in "${selected_cases[@]}"; do
  ...
  [[ -s "$elf" ]] || fail_campaign 1 "Comparison ELF missing: $elf"
  arm-none-eabi-nm -g "$elf" | grep -q 'g_stm32h755_spwkit_profile' || \
    fail_campaign 1 "Comparison ELF lacks g_stm32h755_spwkit_profile"
  ...
  (
    ...
    timeout "${DEBUG_TIMEOUT}s" "$GDB_BIN" -q "$elf" -batch \
      -x "$ROOT_DIR/scripts/gdb/stm32h755_native_compare.gdb"
  ) 2>&1 | tee "$gdb_log"
  gdb_rc=${PIPESTATUS[0]}
  set -e
  cleanup_openocd
  ...
  if (( gdb_rc != 0 )) || ! grep -q '^RESULT: PASS$' "$gdb_log"; then
    status="$gdb_rc"
    (( status == 0 )) && status=1
    fail_campaign "$status" "Native differential case $case_name failed"
  fi
  [[ -s "$raw_out" ]] || fail_campaign 1 "Bulk native comparison evidence dump missing for $case_name"

  python3 "$ROOT_DIR/scripts/extract_stm32h755_native_compare.py" "$gdb_log" \
    --raw "$raw_out" \
    --case "$case_name" --start "$START" --end "$END" \
    --git-sha "$git_sha" --cube-sha "$CUBE_SHA" --compiler "$compiler" \
    --output "$json_out" || fail_campaign $? "Native comparison extraction failed for $case_name"
done
...
meta = {
    'schema': 'spwkit.profile.stm32h755-native-campaign.v1',
    ...
    'pair_order': 'alternating native-first / SpWKit-first',
    ...
    'scope': 'direct STM32 provider callbacks versus SpWKit public API; same DMA2 memory-to-memory provider; not SpaceWire PHY/link timing',
}
(root / 'campaign.json').write_text(json.dumps(meta, indent=2) + '\n')
PY

printf '\n============= STM32H755 NATIVE DIFFERENTIAL RESULTS =============\n' >&2
python3 "$ROOT_DIR/scripts/summarize_stm32h755_native_compare.py" "$output_dir" | tee "$output_dir/summary.console.txt" >&2
printf '==================================================================\n' >&2

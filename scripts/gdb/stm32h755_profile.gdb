...
# Keep debugger diagnostics restricted to symbols whose types are emitted by
# the profiling firmware itself. spw_profile_state belongs to the separately
# built static SpWKit library and may legitimately have no GDB type metadata
# for probe pairs that are implemented entirely inside that library.
printf "PROFILE_DEBUG tx_owner=%u rx_owner=%u transfer_length=%u dma_transfers=%u tx_packets=%u rx_packets=%u\n", (unsigned int)g_driver.tx_owner, (unsigned int)g_driver.rx_owner, (unsigned int)g_driver.transfer_length, (unsigned int)g_driver.dma_transfers, (unsigned int)g_driver.statistics.tx_packets, (unsigned int)g_driver.statistics.rx_packets

...
The STM32H755 integration validates that the generic driver/DMA ownership model can be exercised on real Cortex-M7 silicon with actual DMA and explicit cache/coherency handling.

The integration now contains:

- standalone NUCLEO-H755ZI-Q/Cortex-M7 evidence firmware;
...
- a documented board execution/recording procedure.

The remaining requirement is the literal flash-and-run result on physical STM32H755 hardware. A successful run is **MCU DMA/cache evidence**, not physical SpaceWire electrical HIL.

...
- SpaceWire electrical compliance;
- ECSS qualification;
- radiation tolerance;
...
Evidence must therefore state exactly which layer was exercised.
...
Passing these tests proves interoperability for the recorded configuration. It is not a blanket electrical or ECSS qualification claim.
...
| Qualification/compliance campaign | Only the requirements explicitly covered by that campaign. |
...
This keeps the public API portable without pretending every underlying implementation has identical resources or performance.
...
enum {
    ...
    SPW_VSPW_TIME_CODE_QUEUE_DEPTH = 8u,
    SPW_VSPW_RECENT_MESSAGE_DEPTH = 32u,
    SPW_VSPW_RETIRED_SESSION_DEPTH = 8u,
    SPW_VSPW_CONTROL_MESSAGE_SIZE = 64u
};
...
typedef struct spw_vspw_engine {
    ...
    spw_time_code_t time_codes[SPW_VSPW_TIME_CODE_QUEUE_DEPTH];
    ...
    spw_vspw_delivered_key_t recent_messages[SPW_VSPW_RECENT_MESSAGE_DEPTH];
    size_t recent_message_head;
    size_t recent_message_count;

    uint64_t retired_sessions[SPW_VSPW_RETIRED_SESSION_DEPTH];
...
bool spw_vspw_engine_supports_zero_copy(const spw_vspw_engine_t* engine);

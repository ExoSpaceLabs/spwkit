...
static spw_result_t validate_common_config(const spw_port_config_t* config) {
    ...
    if (config->struct_size < SPW_PORT_CONFIG_MIN_SIZE) {
        return SPW_ERR_INVALID_ARGUMENT;
    }
...
static spw_result_t validate_device_config(const spw_port_config_t* config) {
    ...
    if (config->backend_config == NULL ||
        config->backend_config_size < SPW_DEVICE_CONFIG_MIN_SIZE) {
        return SPW_ERR_INVALID_ARGUMENT;
    }
    device = (const spw_device_config_t*)config->backend_config;
    if (device->struct_size < SPW_DEVICE_CONFIG_MIN_SIZE ||
...
static spw_result_t validate_driver_config(const spw_port_config_t* config) {
    ...
    if (config->backend_config == NULL ||
        config->backend_config_size < SPW_DRIVER_CONFIG_MIN_SIZE) {
        return SPW_ERR_INVALID_ARGUMENT;
    }
    driver = (const spw_driver_config_t*)config->backend_config;
    if (driver->struct_size < SPW_DRIVER_CONFIG_MIN_SIZE ||
    ...
    ops = driver->ops;
    if (ops->struct_size < SPW_DRIVER_OPS_MIN_SIZE) {
        return SPW_ERR_INVALID_ARGUMENT;
    }
...
static spw_result_t validate_simulator_config(const spw_port_config_t* config) {
    ...
    if (config->backend_config == NULL ||
        config->backend_config_size < SPW_SIMULATOR_CONFIG_MIN_SIZE) {
        return SPW_ERR_INVALID_ARGUMENT;
    }

    simulator = (const spw_simulator_config_t*)config->backend_config;
    if (simulator->struct_size < SPW_SIMULATOR_CONFIG_MIN_SIZE ||
...
static spw_result_t validate_udp_config(const spw_port_config_t* config) {
    ...
    size_t i;
    if (config->backend_config == NULL ||
        config->backend_config_size < SPW_UDP_CONFIG_MIN_SIZE) {
        return SPW_ERR_INVALID_ARGUMENT;
    }

    udp = (const spw_udp_config_t*)config->backend_config;
    if (udp->struct_size < SPW_UDP_CONFIG_MIN_SIZE ||
...
static spw_result_t validate_raw_ethernet_config(
    ...
    if (config->backend_config == NULL ||
        config->backend_config_size < SPW_RAW_ETHERNET_CONFIG_MIN_SIZE) {
        return SPW_ERR_INVALID_ARGUMENT;
    }
    raw = (const spw_raw_ethernet_config_t*)config->backend_config;
    if (raw->struct_size < SPW_RAW_ETHERNET_CONFIG_MIN_SIZE ||
    ...
    io = raw->io_ops;
    if (io->struct_size < SPW_RAW_ETHERNET_IO_OPS_MIN_SIZE) {
        return SPW_ERR_INVALID_ARGUMENT;
    }
    ...
    if (runtime->struct_size < SPW_RUNTIME_OPS_MIN_SIZE) {
        return SPW_ERR_INVALID_ARGUMENT;
    }
